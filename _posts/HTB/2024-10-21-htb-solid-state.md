---
layout: post
title: HTB Bashed Write Up
subtitle: 
date: 2024-10-21
author: Mike Vedete
category: HTB
tags: [ctf,pop3,apache james]
---

SolidState from Hackthebox, is a medium machine that requires chaining a few attacks to obtain a user shell. Once a shell is obtained privilege escalation is straightforward after proper enumeration. 

# Enumeration

Initial enumeration of the machine was done with: `nmap -sC -sV -oN nmap 10.129.209.227`
```bash
...
![phpbash homepage](assets/img/htb/bashed/phpbash.png)
...
```
# Other Observations
In an effort to understand what is really happening, we can take a look at cron jobs that run as the root user. Running `crontab -l` will show us a bash script that runs every minute is configured. What I found interesting is that the script will change directories to `/scripts` then execute every file ending in .py. This was unknown but we could have named the file anything we wanted, as long as it ended in `.py`. 
```bash
crontab -l
crontab -l
