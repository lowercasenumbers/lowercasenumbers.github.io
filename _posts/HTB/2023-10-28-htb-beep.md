...
```
Review of the initial nmap scan we identify that port 80 is open. More often than not we should start enumerating directories while we look at something else. So to start we kick off gobuster again port 80 
