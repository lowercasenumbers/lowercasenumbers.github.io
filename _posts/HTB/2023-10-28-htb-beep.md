...
category: HTB
---
Beep from Hackthebox, is a fairly easy machine that multiple exploitation paths. The patch we will take is direct to root and involves a little enumeration, identifying an exploit and a few good guesses.  
# Enumeration
...
```
Review of the initial nmap scan we identify that port 80 is open. More often than not we should start enumerating directories while we look at something else. So to start we kick off gobuster agains port 80 

n order to multitask, we start another scan with `gobuster` to try to identify any subdirectories. We will use the arguments `dir, --url, --wordlist, --o`
```bash
...
![[../../Screenshots/Pasted image 20231028160029.png]]
...
![[../../Screenshots/Pasted image 20231028160604.png]]
